...
class CandyxPEBot:
    ...
    def load_rules(self):
        try:
            ...
            with open('candyxpe_rules.txt', 'r', encoding='utf-8') as file:
                rules = file.read().strip() or "Правила сервера CandyxPE отсутствуют."
                if len(rules) > 1000:
    ...
    def load_languages(self):
        try:
            file_path = 'candyxpe_languages.json'
    ...
    def load_agents(self):
        try:
            file_path = 'candyxpe_agents.json'
            if not os.path.exists(file_path):
                with open(file_path, 'w', encoding='utf-8') as file:
                    json.dump({}, file)
            with open(file_path, 'r', encoding='utf-8') as file:
    ...
    def get_keyboard(self, mode="main", user_id=None):
        ...
        keyboards = {
            ...
            "ai": {
                "ru": {"one_time": False, "buttons": [[{"action": {"type": "text", "payload": "{\"command\": \"end_ai\"}", "label": "🔙 Выйти из ИИ"}, "color": "negative"}]]},
                "en": {"one_time": False, "buttons": [[{"action": {"type": "text", "payload": "{\"command\": \"end_ai\"}", "label": "🔙 Exit AI"}, "color": "negative"}]]}
            },
            "human": {
                "ru": {"one_time": False, "buttons": [[{"action": {"type": "text", "payload": "{\"command\": \"end_human\"}", "label": "🔙 Назад"}, "color": "negative"}]]},
                "en": {"one_time": False, "buttons": [[{"action": {"type": "text", "payload": "{\"command\": \"end_human\"}", "label": "🔙 Back"}, "color": "negative"}]]}
            },
            "action": {
                "ru": {"one_time": False, "buttons": [[{"action": {"type": "text", "payload": "{\"command\": \"cancel\"}", "label": "❌ Отмена"}, "color": "negative"}]]},
                "en": {"one_time": False, "buttons": [[{"action": {"type": "text", "payload": "{\"command\": \"cancel\"}", "label": "❌ Cancel"}, "color": "negative"}]]}
            },
            "admin": {
                "ru": {
                    "one_time": False,
        ...
        try:
            return keyboards.get(mode, keyboards["main"])[language]
    ...
    def send_to_admin_chat(self, message: str, user_id: int, action_type: str, attachments=None):
        error_messages = {
            917: "❌ Сообщество не имеет прав администратора в чате. Добавьте сообщество в чат.",
            912: "❌ Включите функцию чат-бота в настройках сообщества!",
            27: "❌ Ошибка: метод недоступен для токена сообщества. Используйте токен пользователя или проверьте настройки."
        }
        try:
            ...
            if not response.get('items'):
                ...
                self.send_message(user_id, f"❌ Чат с ID {self.admin_chat_id} недоступен.", self.get_keyboard("main", user_id))
                return False
        except vk_api.exceptions.ApiError as e:
            logger.error(f"Ошибка доступа к чату {self.admin_chat_id}: {e} (Код: {e.code})", extra={'user_id': user_id})
            self.send_message(user_id, error_messages.get(e.code, f"❌ Ошибка доступа к чату: {str(e)} (Код: {e.code})."), self.get_keyboard("main", user_id))
            return False
        ...
        try:
            ...
            logger.error(f"Ошибка отправки в админ-чат: {e} (Код: {e.code})", extra={'user_id': user_id})
            self.send_message(user_id, error_messages.get(e.code, "❌ Админ-чат недоступен."), self.get_keyboard("main", user_id))
            return False
        except Exception as e:
    ...
    def send_broadcast(self, message: str, sender_id: int):
        ...
        for user_id, user_language in self.user_languages.items():
            if int(user_id) not in self.banned_users:
                try:
                    self.send_message(int(user_id), f"📢 {'Объявление от CandyxPE' if user_language == 'ru' else 'Announcement from CandyxPE'}:\n{message}")
                    sent_count += 1
                except Exception as e:
    ...
    def send_message(self, user_id: int, message: str, keyboard=None, attachment=None):
        ...
        messages = {
            "ru": {
                "welcome": "👋 Добро пожаловать в бота технической поддержки CandyxPE!\nВыберите действие:",
        ...
        try:
            params = {
                'user_id': user_id,
                'message': messages[language].get(message, message),
                'random_id': get_random_id()
            }
            if keyboard:
            ...
            logger.error(f"Неизвестная ошибка отправки: {e}", extra={'user_id': user_id})
    ...
    def get_ai_response(self, user_id: int, user_message: str):
        ...
        try:
            ...
            if isinstance(response, str) and response.strip():
                ...
                return response
            logger.error(f"Некорректный ответ от g4f: {response}", extra={'user_id': user_id})
            return "❌ Ошибка ИИ. Попробуйте позже или обратитесь к агенту." if language == "ru" else "❌ AI error. Try again later or contact an agent."
        except Exception as e:
            logger.error(f"Ошибка в get_ai_response: {e}", extra={'user_id': user_id})
            return "❌ Ошибка. Обратитесь к поддержке CandyxPE." if language == "ru" else "❌ Error. Contact CandyxPE support."
    ...
    def handle(self, user_id: int, command: str):
        ...
        commands = {
            "ai_agent": lambda: (
                ...
                self.send_message(user_id, "ai_activated", self.get_keyboard("ai", user_id))
            ),
            "contact_agent": lambda: (
                ...
                self.send_message(user_id, "human_mode", self.get_keyboard("human", user_id))
            ),
            "end_human": lambda: (
                self.user_human_mode.discard(user_id),
                self.send_message(user_id, "human_exit", self.get_keyboard("main", user_id))
            ),
            "report_staff": lambda: (
                self.user_action_mode.update({user_id: "staff"}),
                self.user_human_mode.discard(user_id),
                self.send_message(user_id, "report_staff", self.get_keyboard("action", user_id))
            ),
            "report_bug": lambda: (
                self.user_action_mode.update({user_id: "bug"}),
                self.user_human_mode.discard(user_id),
                self.send_message(user_id, "report_bug", self.get_keyboard("action", user_id))
            ),
            "change_language": lambda: (
                self.user_languages.update({str(user_id): "en" if self.user_languages.get(str(user_id), "ru") == "ru" else "ru"}),
                self.save_languages(),
                self.send_message(user_id, "language_changed".format(language="Русский" if self.user_languages[str(user_id)] == "ru" else "English"), self.get_keyboard("main", user_id))
            ),
            "end_ai": lambda: (
                self.user_ai_mode.discard(user_id),
                ...
                self.user_human_mode.discard(user_id),
                self.send_message(user_id, "exit_ai", self.get_keyboard("main", user_id))
            ),
            "cancel": lambda: (
                self.user_action_mode.pop(user_id, None),
                self.user_ai_mode.discard(user_id),
                self.user_human_mode.discard(user_id),
                self.send_message(user_id, "cancel", self.get_keyboard("main", user_id))
            ),
            "admin_panel": lambda: self.send_message(user_id, "admin_panel", self.get_keyboard("admin", user_id)) if self.is_agent(user_id) else self.send_message(user_id, "admin_denied", self.get_keyboard("main", user_id)),
            "manage_agents": lambda: self.send_message(user_id, "manage_agents", self.get_keyboard("manage_agents", user_id)) if self.is_admin(user_id) else self.send_message(user_id, "admin_denied", self.get_keyboard("admin", user_id)),
            "ban_user": lambda: self.send_message(user_id, "ban_user", self.get_keyboard("ban_user", user_id)) if self.is_admin(user_id) else self.send_message(user_id, "admin_denied", self.get_keyboard("admin", user_id)),
            "broadcast": lambda: (
                self.user_action_mode.update({user_id: "broadcast"}),
                self.send_message(user_id, "broadcast", self.get_keyboard("action", user_id))
            ) if self.is_admin(user_id) else self.send_message(user_id, "admin_denied", self.get_keyboard("admin", user_id)),
            "add_agent": lambda: (
                self.user_action_mode.update({user_id: "add_agent"}),
                self.send_message(user_id, "add_agent", self.get_keyboard("action", user_id))
            ) if self.is_admin(user_id) else self.send_message(user_id, "admin_denied", self.get_keyboard("admin", user_id)),
            "remove_agent": lambda: (
                self.user_action_mode.update({user_id: "remove_agent"}),
                self.send_message(user_id, "remove_agent", self.get_keyboard("action", user_id))
            ) if self.is_admin(user_id) else self.send_message(user_id, "admin_denied", self.get_keyboard("admin", user_id)),
            "ban": lambda: (
                self.user_action_mode.update({user_id: "ban"}),
                self.send_message(user_id, "ban", self.get_keyboard("action", user_id))
            ) if self.is_admin(user_id) else self.send_message(user_id, "admin_denied", self.get_keyboard("admin", user_id)),
            "unban": lambda: (
                self.user_action_mode.update({user_id: "unban"}),
                self.send_message(user_id, "unban", self.get_keyboard("action", user_id))
            ) if self.is_admin(user_id) else self.send_message(user_id, "admin_denied", self.get_keyboard("admin", user_id))
        }
        commands.get(command, lambda: self.send_message(user_id, "unknown_command", self.get_keyboard("main", user_id)))()

    def process_message(self, event):
        user_id = event.user_id
        if event.from_chat or not event.to_me:
            return
        language = self.user_languages.get(str(user_id), "ru")
        ...
        try:
            ...
            if user_id in self.user_human_mode:
                attachments = [f"{att['type']}{att[att['type']]['owner_id']}_{att[att['type']]['id']}" for att in event.attachments.get("attachments", []) if att["type"] in ["photo", "video"]]
            ...
            if user_id in self.user_action_mode:
                ...
                if action_type in ["staff", "bug"]:
                    attachments = [f"{att['type']}{att[att['type']]['owner_id']}_{att[att['type']]['id']}" for att in event.attachments.get("attachments", []) if att["type"] in ["photo", "video"]]
                    report_type = "Жалоба" if action_type == "staff" else "Сообщение о баге"
                    report_type_en = "Complaint" if action_type == "staff" else "Bug report"
                    if self.send_to_admin_chat(text, user_id, action_type, attachments):
                        self.send_message(user_id, "report_sent".format(type=report_type if language == "ru" else report_type_en), self.get_keyboard("main", user_id))
                    else:
                        self.send_message(user_id, "report_failed".format(type=report_type.lower() if language == "ru" else report_type_en.lower()), self.get_keyboard("main", user_id))
                    self.user_action_mode.pop(user_id, None)
                elif action_type == "broadcast":
                    if self.send_broadcast(text, user_id):
                        self.send_message(user_id, "broadcast_sent", self.get_keyboard("admin", user_id))
                    else:
                        self.send_message(user_id, "broadcast_failed", self.get_keyboard("admin", user_id))
                    self.user_action_mode.pop(user_id, None)
                    ...
                    try:
                        ...
                        if agent_id == user_id:
                            self.send_message(user_id, "self_agent", self.get_keyboard("manage_agents", user_id))
                        elif str(agent_id) in self.agents:
                            self.send_message(user_id, "already_agent".format(agent_id=agent_id), self.get_keyboard("manage_agents", user_id))
                        else:
                            self.agents[str(agent_id)] = {"role": role}
                            self.save_agents()
                            self.send_message(user_id, "agent_added".format(role=role.capitalize(), agent_id=agent_id), self.get_keyboard("admin", user_id))
                            self.send_message(agent_id, "agent_added_notify".format(role=role), self.get_keyboard("main", agent_id))
                            self.send_to_admin_chat(f"{role.capitalize()} id{agent_id} назначен.", user_id, "add_agent")
                        self.user_action_mode.pop(user_id, None)
                    except ValueError:
                        self.send_message(user_id, "invalid_format".format(format="<ID> <agent/admin/manager>", example="123456 agent"), self.get_keyboard("action", user_id))
                elif action_type == "remove_agent":
                    try:
                        agent_id = int(text)
                        if agent_id == user_id:
                            self.send_message(user_id, "self_remove", self.get_keyboard("manage_agents", user_id))
                        elif str(agent_id) in self.agents:
                            role = self.agents[str(agent_id)].get("role", "agent")
                            del self.agents[str(agent_id)]
                            self.save_agents()
                            self.send_message(user_id, "agent_removed".format(role=role.capitalize(), agent_id=agent_id), self.get_keyboard("admin", user_id))
                            self.send_message(agent_id, "agent_removed_notify".format(role=role), self.get_keyboard("main", agent_id))
                            self.send_to_admin_chat(f"{role.capitalize()} id{agent_id} снят.", user_id, "remove_agent")
                        else:
                            self.send_message(user_id, "not_agent".format(agent_id=agent_id), self.get_keyboard("manage_agents", user_id))
                        self.user_action_mode.pop(user_id, None)
                    except ValueError:
                        self.send_message(user_id, "invalid_id", self.get_keyboard("action", user_id))
                    ...
                    try:
                        ...
                        if target_id == user_id:
                            self.send_message(user_id, "self_ban", self.get_keyboard("ban_user", user_id))
                        elif self.is_agent(target_id):
                            self.send_message(user_id, "agent_ban", self.get_keyboard("ban_user", user_id))
                        else:
                            self.banned_users[target_id] = datetime.now() + timedelta(hours=hours)
                            self.send_message(user_id, "banned".format(target_id=target_id, hours=hours), self.get_keyboard("ban_user", user_id))
                            self.send_message(target_id, "banned_notify".format(hours=hours), self.get_keyboard("main", target_id))
                            self.send_to_admin_chat(f"id{target_id} забанен на {hours} часов.", user_id, "ban")
                        self.user_action_mode.pop(user_id, None)
                    except ValueError:
                        self.send_message(user_id, "invalid_format".format(format="<ID> <часы>", example="123456 24"), self.get_keyboard("action", user_id))
                elif action_type == "unban":
                    try:
                        target_id = int(text)
                        if target_id in self.banned_users:
                            del self.banned_users[target_id]
                            self.send_message(user_id, "unbanned".format(target_id=target_id), self.get_keyboard("ban_user", user_id))
                            self.send_message(target_id, "unbanned_notify", self.get_keyboard("main", user_id))
                            self.send_to_admin_chat(f"id{target_id} разбанен.", user_id, "unban")
                        else:
                            self.send_message(user_id, "not_banned".format(target_id=target_id), self.get_keyboard("ban_user", user_id))
                        self.user_action_mode.pop(user_id, None)
                    except ValueError:
                        self.send_message(user_id, "invalid_id", self.get_keyboard("action", user_id))
                return
            ...
            if text.lower() in ["начать", "привет", "start", "hello"]:
                self.send_message(user_id, "welcome", self.get_keyboard("main", user_id))
            else:
                self.send_message(user_id, "unknown_command", self.get_keyboard("main", user_id))
        except Exception as e:
            logger.error(f"Ошибка обработки сообщения: {e}", extra={'user_id': user_id})
            self.send_message(user_id, "error", self.get_keyboard("main", user_id))
